...
<div class="controls">
    <label for="revealColorPicker">Choose Revealed Square Color:</label>
    <input type="color" id="revealColorPicker" value="#FFFFFF">
...
    const memCanvas = document.getElementById('memoryCanvas');
    const memCtx = memCanvas.getContext('2d');
    const colorPicker = document.getElementById('revealColorPicker'); // New element
...
    const highScoreDisplay = document.getElementById('highScoreDisplay');
    ...
    let selectedRevealColor = '#FFFFFF'; 

    let score = 0; // Player's score
    let attempts = 0; // Number of attempts made

    // --- INITIALIZATION AND HIGH SCORE LOGIC ---
...
        memCtx.lineWidth = 10;
        ...
        // Draw vertical lines
        for (let x = 0; x <= canvasWidth; x += canvasWidth / cols) {
            memCtx.beginPath();
            memCtx.moveTo(x, 0);
            memCtx.lineTo(x, canvasHeight);
            memCtx.stroke();
        ...
        // Draw horizontal lines
        for (let y = 0; y <= canvasHeight; y += canvasHeight / rows) {
            memCtx.beginPath();
            memCtx.moveTo(0, y);
            memCtx.lineTo(canvasWidth, y);
            memCtx.stroke();
...
        // *** COLOR FEATURE IMPLEMENTATION: Use the global selectedRevealColor ***
        memCtx.fillStyle = selectedRevealColor; 
        memCtx.fillRect(col * cellWidth + 5, row * cellHeight + 5, cellWidth - 10, cellHeight - 10);
        memCtx.fillStyle = '#000000';
        memCtx.fillText(emoji, x, y);
        return emoji;
...
    });
    ...
    function startGame() {
        // 1. Get the color selected by the user
        selectedRevealColor = colorPicker.value;
        
        // 2. Restart the game with the chosen color (and a new shuffle)
        location.reload(); 
        
        // If you want to change the color *without* reloading, you'd need to clear the canvas,
        // re-shuffle, reset scores, and re-draw everything. For simplicity and consistency 
        // with your win condition logic, a reload is easier here.
    }
